...
int parse_http_request_file(char * file_path, struct http_compare_point * http_compare_point)
{
    ...
    memset(&http_file, 0, sizeof(struct file));
    ...
    if ( http_compare_point == NULL ){
        printf("http_compare_point 매개변수 전달 에러입니다.\n");
        return -1;
    }
    if ( file_path == NULL ){
        printf("file_path 매개변수 전달 에러입니다.\n");
        return -1;
    }

    host = "Host";

    result = read_file(file_path, &http_file);
	if ( result == -1 ){
		printf("read_fild 에러, 프로그램 종료\n");
		if ( http_file.file_pointer != NULL ){
		free(http_file.file_pointer);
		}
		return -1;
	}
    ...
    request_start_line = http_file.file_pointer;
    written_size = http_file.file_size;
    ...
    /* path 값 복사 */
    http_compare_point->path_length = URI.path_length;
    http_compare_point->path = malloc(http_compare_point->path_length + 1); /* NULL */
    if ( http_compare_point->path == NULL ){
        printf("path 메모리 할당 오류입니다.\n");
        return -1;
    }
    memset(http_compare_point->path, 0, http_compare_point->path_length + 1); /* NULL */
    memcpy(http_compare_point->path, URI.path, http_compare_point->path_length);
    ...
    temp_header = header_list.header_head;

    while(temp_header != NULL){
        
        printf("name : %s\n", temp_header->name);
        result = strncmp(temp_header->name, host, strlen(host));
        if ( result == 0 ){

            printf("host 찾았다~!\n");
            http_compare_point->host_length = temp_header->value_length;
            http_compare_point->host = malloc(http_compare_point->host_length + 1); /* NULL */
            ...
            memset(http_compare_point->host, 0, http_compare_point->host_length + 1); /* NULL */
            memcpy(http_compare_point->host, temp_header->value, http_compare_point->host_length);
    ...
    if ( http_compare_point->host == NULL ){
        printf("host 값은 없습니다..\n");
        http_compare_point->host = NULL;
        http_compare_point->host_length = 0;
    }
